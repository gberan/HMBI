...
!! $Rev: 414 $
!! $Author: gberan $
!! $URL: http://ccpforge.cse.rl.ac.uk/svn/dl-find/branches/release_chemsh3.3/dlf_hdlc_interface.f90 $
!! $Id: dlf_hdlc_interface.f90,v 1.1 2010-05-07 19:30:43 gberan Exp $
!!
!! COPYRIGHT
!!
...
!!
!! SYNOPSIS
subroutine dlf_hdlc_get_nivar(nivar)
!! SOURCE
...
! **********************************************************************
  ...
  nivar=glob%nat * 3 - hdlc%nconstr - hdlc%nmin - hdlc%nfrozen * 3
...
!!
!! SYNOPSIS
subroutine dlf_hdlc_create(nat,spec,attypes,nimage,xcoords,xweight,mass)
!! SOURCE
  ...
  integer             :: i,j,fail,ncart,maxcon,iimage,iphdlc
...
!!
!! SYNOPSIS
subroutine dlf_hdlc_getweight(nat,nivar,xweight,iweight)
!! SOURCE
  use dlf_parameter_module, only: rk
  use dlf_global, only: printl,stdout  
  use dlfhdlc_hdlclib
...
! **********************************************************************
  ...
  DO group = 1, hdlc%ngroups
    ...
    iphdlc = residue%ip + ndfhdlc-hdlc%nmin ! for correct start of cartesians
  end DO

  DO iat = 1, nat
    IF (hdlc%resn(iat)==0) THEN
      iweight(iphdlc:iphdlc+2)=xweight(iat)
      iphdlc=iphdlc+3
    end IF
  end DO
...
!!
!! SYNOPSIS
subroutine dlf_hdlc_xtoi(nat,nivar,xcoords,xgradient,icoords,igradient)
!! SOURCE
  use dlf_parameter_module, only: rk
  use dlf_global, only: printl,stdout  
  use dlf_allocate, only: allocate,deallocate
...
!!
!! SYNOPSIS
subroutine dlf_hdlc_itox(nat,nivar,icoords,xcoords,tok)
!! SOURCE
  use dlf_parameter_module, only: rk
  use dlf_global, only: printl,stdout  
...
! prepare for the next group
    iphdlc = iphdlc + ndfhdlc-hdlc%nmin ! for correct cartesian start
    idum = matrix_destroy(cxyz)
    idum = matrix_destroy(chdlc)
    call deallocate(atmp)
...
!  this should be deallocated. However, due to a compiler bug in pgf90
!  the deallocation is avoided.
